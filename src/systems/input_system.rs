    ...
    for event in input.iter() {
        if event.state.is_pressed() {
            if let Some(input_event) = match event.key_code {
                ...
                Some(KeyCode::Return) => Some(events::InputEvent::Shoot),
                _ => None,
            } {
                match input_event {
            ...
            if let Some(input_event) = match event.key_code {
                ...
                if !input_store.player_move_north
                    && !input_store.player_move_south
                    && !input_store.player_move_east
                    && !input_store.player_move_west
                {
